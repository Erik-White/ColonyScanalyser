...
def plot_appearance_frequency_all(plate_list, plate_lattice, time_points_elapsed, save_path):
    """
    ...
    for plate_item in plate_list.items():
        ...
        # Add the growth curve plot for this plate
        time_of_appearance_frequency(ax, plate_item, time_points_elapsed, cm_plate)
    ...
    fig.savefig(str(save_path.joinpath("time_of_appearance_comparison.png")), **save_params)

    plt.close()


def plot_appearance_frequency(plate_item, time_points_elapsed, save_path):
    """
    Time of appearance frequency for a single plate
    """
    fig, ax = plt.subplots()

    # Plot areas for each colony
    time_of_appearance_frequency(ax, plate_item, time_points_elapsed, "Purple")
    ...
    plt.ylim(ymin = 0)
    plt.title("Time of appearance")
    fig.savefig(str(save_path.joinpath("time_of_appearance.png")), **save_params)

    plt.close()


def time_of_appearance_frequency(ax, plate_item, time_points_dict, line_color):
    """
    Add a time of appearance frequency plot to an axis
    """
    plate_id, plate = plate_item

    # Map counts to a dictionary of all timepoints
    time_points_dict = dict.fromkeys(time_points_dict, 0)
    for colony in plate.values():
        time_points_dict[colony.timepoint_first.elapsed_minutes] += 1
